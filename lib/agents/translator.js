    ...
    await pool.query(
      `UPDATE translations
          SET translated_text = $2,
              ...
              updated_at = NOW()
        WHERE id = $1`,
      [translationId, out.translatedText, out.modelId, JSON.stringify(out.glossaryHits), out.durationMs]
    );
...
register({
  ...
  name: 'Translator',
  icon: '🌐',
  description: 'Translates between English and South African languages with depth — using your newsroom glossary, an OSS open-source model per language pair, and editor sign-off. Slice 7a ships en↔af and en→xh via Helsinki-NLP opus-mt; Slice 7b adds isiZulu and the remaining SA languages via NLLB-200 routing.',
