...
const BASE_SYSTEM = `You are Grounded's Copywriter agent — an editorial assistant for African newsrooms.
...
async function draft({
  ...
  // Read newsroom profile (voice, style, audience, ethics) so the agent
  // doesn't draft in a generic voice. Falls through gracefully if no
  // profile has been authored yet — the agent still works, just without
  // newsroom-specific shaping.
  let profileBlock = null;
  try {
    if (context?.newsroomId) {
      const profile = await loadProfile(context.newsroomId);
      profileBlock = formatForPrompt(profile);
    }
  } catch (e) {
    // non-fatal — log and proceed
    console.error('drafter: profile load failed', e);
  }

  // Optional topic-match hint (Step 0 of the consolidation plan).
  // Gated by GROUNDED_TOPIC_HINT=on so we can review real outputs before flipping.
  // With the flag off, userMessage is byte-identical to before this change.
  let topicHint = null;
  if (process.env.GROUNDED_TOPIC_HINT === 'on') {
    try {
      const { getEffectiveTopics, scoreArticle, formatScoreLine } = require('../copywriter/topic-tags');
      const topics = await getEffectiveTopics(context?.newsroomId || null);
      const score = scoreArticle(articleText, topics);
      const line = formatScoreLine(score);
      if (line) topicHint = line;
    } catch (e) {
      // non-fatal — log and proceed
      console.error('drafter: topic-match hint failed', e);
    }
  }

  const baseMessage = profileBlock
    ? `--- NEWSROOM CONTEXT ---\n${profileBlock}\n\n--- ARTICLE ---\n\n${articleText}\n\n${instructions}\n\nReturn JSON only.`
    : `Article:\n\n${articleText}\n\n${instructions}\n\nReturn JSON only.`;
  // Hint sits between newsroom context and article when present; never alters
  // the no-hint base message.
  const userMessage = topicHint
    ? baseMessage.replace(/(--- ARTICLE ---|Article:)/, `--- TOPIC MATCH ---\n${topicHint}\n\n$1`)
    : baseMessage;
...
register({
  ...
  name: 'Copywriter',
  icon: '✍️',
  description: 'Writes social copy, headlines, newsletter blurbs, and scripts in the newsroom\'s house style. Post these easily to social media accounts. Handles light translation. In short-form video, paste an article and Copywriter produces the script (opening hook included) for Translator and the Audio & Video Producer to take forward.',
  ...
  inputs: {
    articleText: {
      ...
      description: 'The article Copywriter writes from. Either filled by the user when they run the workflow, or wired from another node.',
    },
  },
  config: {
    task_type: {
    ...
    num_drafts: {
      ...
      description: 'Copywriter returns this many independent drafts so the editor can pick.',
    },
    tone: {
      type: 'select',
    ...
    audience_segment: {
      ...
      description: 'Who this is being written for. Copywriter shapes vocabulary accordingly.',
      placeholder: 'e.g. young township readers, business audience, rural feature-phone users',
    },
  },
  outputs: {
    result: { type: 'json', description: 'Copywriter output: { drafts: [{text, rationale}], editorial_note }.' },
  },
