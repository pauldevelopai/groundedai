...
const SYSTEM_PROMPT = `You are Anchor's Verifier agent — a fact-checking assistant for African newsrooms.
...
3. NO ARCHIVE ACCESS in this version. Do not claim you have cross-referenced the newsroom's archive. External-source citations should reference verifiable public information only — and you must note that the journalist needs to independently confirm them.
 ...
 * @returns {Promise<{ result: object, cost: object, durationMs: number }>}
 */
async function verify({ articleText, specificClaims, context }) {
  if (!articleText || typeof articleText !== 'string' || articleText.length < 50) {
    throw new Error('articleText is required (min 50 chars).');
  }

  const userMessage =
    specificClaims && specificClaims.length > 0
