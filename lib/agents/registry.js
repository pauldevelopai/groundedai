// Agent registry — single source of truth for "what agents Grounded has".
//
...
//   category     — 'agent' | 'tool' (default 'agent'). Per the 2026-05-18
//                  concept-note framing: agents do journalism work
//                  (verify, archive, write, research, translate, produce,
//                  gather, listen); tools support newsroom operations
//                  (fundraise, audience analytics, ops, legal tracking,
//                  security audit). The split surfaces in the GlobalNav
//                  dropdown and the Builder palette. Slugs / tables /
//                  registration are otherwise identical.
...
// Agents whose backends rely on multi-GB LOCAL AI models (downloaded lazily by
// @huggingface/transformers on first use): Archivist (bge-m3 embeddings),
// Translator (NLLB/opus-mt), Producer (Whisper/Piper). On the lean deployment
// these ship as "coming soon" shells — visible in the UI, but their run() is
// stubbed so the models never download. Flip an entry off this set (and provide
// the box resources / hosted embedding) to turn it on. Override via
// GROUNDED_COMING_SOON (comma-separated slugs) if needed.
const COMING_SOON = new Set(
  (process.env.GROUNDED_COMING_SOON !== undefined
    ? process.env.GROUNDED_COMING_SOON.split(',').map((s) => s.trim()).filter(Boolean)
    : ['archivist', 'translator', 'producer'])
);
...
function register(entry) {
  ...
  // config, icon, and category are optional; default to safe values so older agents still register.
  ...
  if (entry.category !== 'agent' && entry.category !== 'tool') {
    throw new Error(`registry.register: category must be 'agent' or 'tool', got "${entry.category}"`);
  }
  // Lean deployment: gate model-heavy agents as "coming soon" — surfaced to the
  // UI via the flag, and the real run() is stubbed so the local models never
  // download (they load lazily inside the real run, so never calling it = never
  // fetching the multi-GB weights).
  entry.comingSoon = COMING_SOON.has(entry.slug);
  if (entry.comingSoon) {
    const realRun = entry.run; // eslint-disable-line no-unused-vars
    entry.run = async () => {
      const err = new Error(`The ${entry.name} agent is coming soon — its local AI models aren't enabled on this deployment yet.`);
      err.code = 'AGENT_COMING_SOON';
      err.status = 503;
      throw err;
    };
  }
...
/** List all registered agents + tools (UI-safe — strips the run function). */
function list() {
  return [..._registry.values()].map(({ run, ...meta }) => meta);
}

/** List entries of a single category. categoryName must be 'agent' or 'tool'. */
function listByCategory(categoryName) {
  return list().filter((e) => e.category === categoryName);
}
...
module.exports = { register, list, listByCategory, get, resolveConfig };
...
require('./operations');
require('./distributor');
require('./social_listener');
require('./legal_tracker');
