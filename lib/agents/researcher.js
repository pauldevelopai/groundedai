...
const AGENTIC_SYSTEM_PROMPT = `You are Grounded's Researcher in agentic mode.

The first-pass analysis has already extracted entities, claims, gaps, and records_to_pull from the document. Your job is to use the tools to enrich that analysis:

- web_fetch the URLs implied by records_to_pull / gaps that are public (court rolls, regulatory disclosures, registry searches).
- archive_search to check whether the newsroom has prior coverage of the same entities, places, or claims.
- (Sparingly) invoke_agent if a downstream agent could resolve a specific gap (e.g. researcher → archivist for a structured archive query).

Hard rules:
1. NEVER ACCUSE. Surface findings as evidence, not verdicts.
2. CITE SOURCES. Quote the fragment that supports each finding.
3. STOP WHEN ENOUGH. Don't keep fetching once the gap is closed.
4. END WITH JSON. Your last message MUST be a single JSON object matching this schema:

{
  "archive_corroboration": [
    { "entity": "<name>", "archive_evidence_count": <int>, "summary": "<one line>" }
  ],
  "web_findings": [
    { "url": "<url>", "title": "<title>", "trusted_source": <bool>, "summary": "<2-sentence summary>", "supports": "<which gap or record_to_pull this closes>" }
  ],
  "remaining_gaps": ["<gap still unresolved after enrichment>"],
  "notes": "<editor-facing 2-3 sentence wrap>"
}
...
/**
 * Run the agentic enrichment over a fresh Researcher result. Pulls
 * web sources for gaps + cross-references entities against the archive.
 * Returns null if there's nothing worth enriching.
 *
 * @param {object} args
 * @param {object} args.result               output of analyzeText()
 * @param {string} args.topic                the research focus (if any)
 * @param {object} args.context              { newsroomId, userId, ... }
 * @param {string} [args.parentInvocationId] workflow_runs.id of the parent
 * @param {number} [args.maxSteps]           default 5
 */
async function runAgenticResearch({
  result, topic, context, parentInvocationId = null, maxSteps = 5,
}) {
  const gaps = Array.isArray(result?.gaps) ? result.gaps : [];
  const records = Array.isArray(result?.records_to_pull) ? result.records_to_pull : [];
  const entities = Array.isArray(result?.entities) ? result.entities : [];
  if (gaps.length === 0 && records.length === 0 && entities.length === 0) return null;

  const payload = {
    topic: topic || null,
    summary: result.summary || null,
    entities: entities.slice(0, 15).map((e) => ({ kind: e.kind, name: e.name, role: e.role })),
    gaps,
    records_to_pull: records,
  };
  const userMessage = `First-pass Researcher output to enrich:\n\n${JSON.stringify(payload, null, 2)}\n\nUse the tools to enrich. End with the JSON object described in your system prompt.`;
  ...
  const loop = await runAgenticLoop({
    ...
    ctx: {
      ...
      endpoint: 'researcher.agentic_enrichment',
    },
    parentInvocationId,
    maxSteps,
  });
  ...
  return {
    supplement: {
      ...
      raw_text: loop.text,
      archive_corroboration: parsed?.archive_corroboration || null,
      web_findings: parsed?.web_findings || null,
      remaining_gaps: parsed?.remaining_gaps || null,
...
module.exports = { analyzeText, runAgenticResearch };
...
register({
  ...
  inputs: {
    documentText: {
      type: 'longtext',
      // NOT required when sourceUrl is supplied (scraper provides the text).
      // The run() function enforces "at least one of {documentText, sourceUrl}".
    ...
    sourceUrl: {
      ...
      description: 'Optional. A URL to scrape and analyse instead of pasting text. The scrape annotates against the trusted-sources list and feeds the result into the same extraction pipeline as documentText.',
    },
    topic: {
      type: 'string',
  ...
  config: {
    ...
    agentic_mode: {
      ...
      description: 'When on, the Researcher autonomously calls web_fetch on candidate URLs implied by gaps + records_to_pull, and archive_search to cross-reference entities, before returning. Adds ~1-3 Haiku calls per run, capped at 5 tool steps.',
    },
  },
  outputs: {
    result: {
  ...
  async run(input, ctx) {
    ...
    // If sourceUrl is supplied, scrape it and use the extracted text as
    // documentText. The scrape result is also appended to the agent's output
    // so consumers can see the trusted-source annotation.
    if (input.sourceUrl && (!documentText || documentText.trim().length < 50)) {
      const { scrapeUrl } = require('../research/scrape');
      const scrape = await scrapeUrl(String(input.sourceUrl), {
        newsroomId: ctx?.newsroomId,
      });
      documentText = scrape.text;
      scrapeMeta = {
        url: scrape.finalUrl,
        title: scrape.title,
        fetchedAt: scrape.fetchedAt,
        contentHash: scrape.contentHash,
        trustedSource: scrape.trustedSource,
        trustedReason: scrape.trustedReason,
      };
    }

    if (!documentText || documentText.trim().length < 50) {
      throw new Error('Either documentText or sourceUrl (with usable content) is required.');
    }
    ...
    // V2 Step 4 — optional agentic enrichment.
    let totalCostUsd = cost?.costUsd ?? 0;
    if (cfg.agentic_mode === true) {
      try {
        const enrichment = await runAgenticResearch({
          result, topic: input.topic, context: ctx, maxSteps: 5,
        });
        if (enrichment) {
    ...
    // Attach scrape metadata to the agent's output when present.
    const out = scrapeMeta ? { result, source: scrapeMeta } : { result };
    return { result: out, cost: { ...cost, costUsd: totalCostUsd }, durationMs };
  },
});
