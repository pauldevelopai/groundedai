// Digital News Gatherer agent (slug stays 'distributor' for back-compat with
// existing workflow definitions). Scope as of 2026-05-15 is inbound-only,
// with three editor-chosen routing destinations (concept-note reconciliation):
//   inbound:    triage incoming submissions, route to one or more of
//               Verifier (fact-check), Researcher (deepen with public
//               records / dossier), Operations (contributor handling or
//               story-idea logging on the editorial calendar).
//
// Two legacy brief kinds (outbound_plan, correction_draft) are retained in
// this file because the outbound dispatch + correction propagation code
// still lives under lib/distribution/* and app/api/distribution/*. Those
// capabilities are now conceptually owned by the Copywriter agent — see
// docs/AGENTS.md. The legacy kinds remain wired so existing workflow
// definitions don't break; the agent registry only advertises the
// inbound_triage kind to new workflows.
//
// All briefs return structured JSON the editor reviews before any
// downstream side effect (creating a contributor row, queueing a send,
// dispatching a correction). Grounded never spawns outbound or contributor
// rows silently — agent proposes, editor confirms.
...
const BASE_SYSTEM = `You are Grounded's Digital News Gatherer agent — the inbound triage assistant for an African newsroom. You receive tips, submissions, and contributor pieces from WhatsApp, web forms, and tip lines, and route them into the editor's queue.
  ...
  "decisions": [
    {
      ...
      "suggested_route": "verifier | researcher | operations_contributor | calendar_idea | archive | block | reply_only",
...
register({
  ...
  triggers: ['tip', 'inbound', 'triage', 'submission', 'contributor'],
  inputs: {
    briefInput: {
      type: 'longtext',
