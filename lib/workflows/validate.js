...
function validateDefinitionShape(def, knownAgentSlugs) {
  ...
  // Empty string is the "unset" sentinel — fresh draft workflows save with
  // output.node = "". The runner enforces a real output at run time.
  if (d.output.node !== '' && !nodeIds.has(d.output.node)) {
