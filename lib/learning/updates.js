...
const DEFAULT_UPDATES = [
  ...
  },
  {
    ...
    body:
      'Anthropic publishes deprecation timelines for Claude model versions. Grounded pins to Claude Haiku 4.5 (id claude-haiku-4-5-20251001) for every agent. When Anthropic announces the eventual deprecation of this model, the Grounded migration plan is to bump to the successor Haiku version and re-test prompts. The local-Ollama fallback (lib/claude.js) covers availability gaps in the meantime.',
    kind: 'model_change',
    severity: 'info',
...
async function listUpdates(newsroomId, opts = {}) {
  ...
  // V2 Step 3: hide pending submissions + archived entries from the public
  // feed. They're surfaced separately via /team review (pending) and an
  // explicit archive view (archived).
  let where = `(u.newsroom_id IS NULL OR u.newsroom_id = $1) AND u.status = 'live'`;
