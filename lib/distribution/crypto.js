...
// loaded from the ANCHOR_DISTRIBUTION_KEY env var (base64).
//
// In production, ANCHOR_DISTRIBUTION_KEY MUST be set and stable — losing
...
// .anchor-distribution-key (gitignored), so dev work doesn't depend on
...
function getKey() {
  ...
  const fromEnv = process.env.ANCHOR_DISTRIBUTION_KEY;
  if (fromEnv) {
    const buf = Buffer.from(fromEnv, 'base64');
    if (buf.length !== 32) {
      throw new Error('ANCHOR_DISTRIBUTION_KEY must be a base64-encoded 32-byte key.');
    }
    cachedKey = buf;
    return cachedKey;
  }
  // Dev fallback — write a stable key file once.
  const keyPath = path.join(process.cwd(), '.anchor-distribution-key');
  ...
  console.warn(
    '[distribution/crypto] No ANCHOR_DISTRIBUTION_KEY env var set — wrote a dev key to ' +
      '.anchor-distribution-key (gitignored). Set the env var in production!'
  );
  cachedKey = fresh;
  return cachedKey;
}
