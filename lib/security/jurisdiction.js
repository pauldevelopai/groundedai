...
function normalisePack(p) {
  // data_law_summary supports two shapes:
  //   - legacy: a plain string
  //   - current: { text, sources[] }
  // The normalised output always carries data_law_summary as a string (the
  // text content) and data_law_sources as an array, so existing callers
  // that .includes() on the summary keep working.
  let summaryText = '';
  let summarySources = [];
  if (typeof p.data_law_summary === 'string') {
    summaryText = p.data_law_summary.trim();
  } else if (p.data_law_summary && typeof p.data_law_summary === 'object') {
    summaryText = typeof p.data_law_summary.text === 'string' ? p.data_law_summary.text.trim() : '';
    summarySources = Array.isArray(p.data_law_summary.sources)
      ? p.data_law_summary.sources.map(normaliseSource).filter(Boolean)
      : [];
  }
  return {
    audit_depth: p.audit_depth === 'deep' ? 'deep' : 'light',
    last_verified: typeof p.last_verified === 'string' ? p.last_verified : null,
    data_law_summary: summaryText,
    data_law_sources: summarySources,
...
function normaliseEntry(e) {
  return {
    ...
    sources: Array.isArray(e.sources) ? e.sources.map(normaliseSource).filter(Boolean) : [],
...
function normaliseSource(s) {
  ...
  return {
    ...
    evidence_kind: typeof s.evidence_kind === 'string' ? s.evidence_kind : null,
    cite: typeof s.cite === 'string' ? s.cite : null,
  };
}
...
function mergePackWithOverrides(pack, overrides) {
  ...
  // Avoid-list: override entries first, then pack entries that aren't
  // replaced by an override. An override entry "replaces" a pack entry
  // when entryMatches() considers them the same target — so an override
  // {vendor: 'OpenAI'} replaces a pack entry {vendor: 'OpenAI', tool_name:
  // 'ChatGPT'} (and any other OpenAI tool), giving the newsroom a vendor-
  ...
  const avoidMerged = [...overrideAvoid];
  for (const packEntry of pack.tool_avoid_list) {
    const replaced = overrideAvoid.some((o) => entryMatches(o, packEntry));
    if (!replaced) avoidMerged.push(packEntry);
  }
