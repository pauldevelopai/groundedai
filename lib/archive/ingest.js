...
async function runPass({ pass, doc, newsroomId, force, context }) {
  ...
  try {
    ...
    if (pass === 'metadata') {
      const r = await extractMetadata({
        documentId: doc.id,
        force,
        context: { ...context, newsroomId },
      });
      rowsAdded = r.fieldsSet.length;
      costUsd = Number(r.cost?.costUsd || 0);
      // If claims have already been extracted for this doc, backfill the
      // newly-extracted asserted_at + byline onto them — the time-aware
      // query layer relies on it.
      if (rowsAdded > 0) {
        await pool.query(
          `UPDATE archive_claims c
              SET asserted_at = d.published_at,
                  byline = d.byline
             FROM archive_documents d
            WHERE c.document_id = d.id AND c.document_id = $1`,
          [doc.id]
        );
      }
    } else if (pass === 'ner') {
...
async function runNerPass({ doc, newsroomId, context }) {
  ...
  for (const chunk of chunks) {
    ...
    // Resolve and insert each mention. Track the entities seen in this
    // chunk so we can apply acronym auto-merge afterwards.
    const chunkEntities = new Map();   // entity_id → { id, canonical_name, type_id, type_slug }
    for (const m of [...universalMentions, ...customMentions]) {
      ...
      rowsAdded++;
      if (!chunkEntities.has(entity.id)) {
        chunkEntities.set(entity.id, {
          id: entity.id,
    ...
    // Acronym auto-merge — runs after the chunk's mentions are written.
    // detectAcronymMerges only emits high-confidence pairs (same type,
    // both entities present in this chunk, exact initials-match, no
    // ambiguity). Editor-mediated merge UI still handles edge cases.
    const merges = detectAcronymMerges({
      chunkText: chunk.text,
      entities: Array.from(chunkEntities.values()),
    });
    for (const directive of merges) {
      try {
        await mergeEntities({
          newsroomId,
          keepId: directive.keepId,
          mergeId: directive.mergeId,
        });
        console.log(`[archive/acronym] merged: ${directive.reason}`);
      } catch (err) {
        // Don't fail the whole NER pass for a merge hiccup
        console.warn(`[archive/acronym] merge failed: ${err.message}`);
      }
    }
  }
