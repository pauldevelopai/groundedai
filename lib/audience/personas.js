...
async function listPersonas(newsroomId) {
  // Soft-deprecated 2026-05-07: synthetic personas are no longer in the
  // Audience agent's scope. We still expose listPersonas + the API for
  // backward compatibility with any long-running workflow nodes that
  // wired through the focus-group runner, but we no longer seed the
  // defaults. New newsrooms get an empty persona list; existing ones
