...
//   • AI-Legal core tables (the AS_IS set) import unchanged — no name collision
//     with GROUNDED's schema.
//   • Every other table gets a `tracker_` prefix. For renamed tables, their
//     constraint and index names are ALSO prefixed, so they can't collide with
//     GROUNDED's identifiers (e.g. the existing funders_pkey).
//   • The legacy `migrations` table — and its sequence — are dropped entirely
//     (GROUNDED's db/migrate.js owns public.migrations).
...
// AI-Legal core — imported AS-IS (no prefix).
const AS_IS = new Set([
...
const EXCLUDE = new Set(['migrations']);
...
const isRenamed = (t) => !AS_IS.has(t) && !EXCLUDE.has(t);
const mapTable = (t) => (EXCLUDE.has(t) ? null : AS_IS.has(t) ? t : 'tracker_' + t);
...
function emitSchema() {
  ...
  const header = [
    ...
    `-- Tables: ${stats.tables} (${stats.asis} as-is, ${stats.renamed} prefixed). Excluded: migrations.`,
    '', '',
  ].join('\n');
