// /api/social/briefs/:id — GET full + PATCH (edited_output / notes / status) + DELETE
...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Hydrate signals so the brief detail page can render them in-line.
  const sigIds = rows[0].signal_ids || [];
  let signals: unknown[] = [];
  if (sigIds.length > 0) {
    const sr = await pool.query(
      ...
      [sigIds, session.newsroomId]
    );
    signals = sr.rows;
  }
  return NextResponse.json({ brief: rows[0], signals });
}
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await ctx.params;
  if (!UUID_RE.test(id)) return NextResponse.json({ error: 'Invalid id' }, { status: 400 });
  const exist = await pool.query(`SELECT id, newsroom_id FROM social_listener_briefs WHERE id = $1`, [id]);
  ...
  values.push(id);
  const { rows } = await pool.query(
    `UPDATE social_listener_briefs SET ${updates.join(', ')} WHERE id = $${values.length} RETURNING *`,
    values
  );
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await pool.query(`DELETE FROM social_listener_briefs WHERE id = $1`, [id]);
