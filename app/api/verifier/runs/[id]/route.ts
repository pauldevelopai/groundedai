// /api/verifier/runs/:id — GET full + PATCH (notes / status / edited_output) + DELETE
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await ctx.params;
  if (!UUID_RE.test(id)) return NextResponse.json({ error: 'Invalid id' }, { status: 400 });
  const exist = await pool.query(`SELECT id, newsroom_id FROM verifier_runs WHERE id = $1`, [id]);
  ...
  values.push(id);
  const { rows } = await pool.query(
    `UPDATE verifier_runs SET ${updates.join(', ')} WHERE id = $${values.length} RETURNING *`,
    values
  );
  return NextResponse.json({ run: rows[0] });
}
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await pool.query(`DELETE FROM verifier_runs WHERE id = $1`, [id]);
