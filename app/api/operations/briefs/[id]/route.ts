// /api/operations/briefs/:id — GET full + PATCH (edited_output / notes / status) + DELETE
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await ctx.params;
  if (!UUID_RE.test(id)) return NextResponse.json({ error: 'Invalid id' }, { status: 400 });
  const exist = await pool.query(`SELECT id, newsroom_id FROM ops_briefs WHERE id = $1`, [id]);
  ...
  const { rows } = await pool.query(
    `UPDATE ops_briefs SET ${updates.join(', ')} WHERE id = $${values.length} RETURNING *`,
    values
  );
  return NextResponse.json({ brief: rows[0] });
}
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await pool.query(`DELETE FROM ops_briefs WHERE id = $1`, [id]);
