// /api/audience/consultations/:id — GET + PATCH + DELETE
...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Hydrate referenced signals so the detail page can show what data
  // grounded the response.
  let referencedSignals: unknown[] = [];
  if (Array.isArray(rows[0].referenced_signal_ids) && rows[0].referenced_signal_ids.length > 0) {
    const sr = await pool.query(
      ...
      [rows[0].referenced_signal_ids, session.newsroomId]
    );
    referencedSignals = sr.rows;
  }
  return NextResponse.json({ consultation: rows[0], referenced_signals: referencedSignals });
}
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await ctx.params;
  if (!UUID_RE.test(id)) return NextResponse.json({ error: 'Invalid id' }, { status: 400 });
  const exist = await pool.query(`SELECT id, newsroom_id FROM audience_consultations WHERE id = $1`, [id]);
  ...
  values.push(id);
  const { rows } = await pool.query(
    `UPDATE audience_consultations SET ${updates.join(', ')} WHERE id = $${values.length} RETURNING *`,
    values
  );
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await pool.query(`DELETE FROM audience_consultations WHERE id = $1`, [id]);
