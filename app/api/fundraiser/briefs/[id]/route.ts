// /api/fundraiser/briefs/:id — GET (full row) + PATCH (edited_output / notes / status) + DELETE
...
const VALID_STATUSES = ['pending', 'generated', 'edited', 'submitted', 'won', 'lost', 'failed'];
...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { rows } = await pool.query(
    `SELECT b.*, f.name AS funder_name, f.application_structure AS funder_application_structure
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await ctx.params;
  if (!UUID_RE.test(id)) return NextResponse.json({ error: 'Invalid id' }, { status: 400 });
  const exist = await pool.query(`SELECT id, newsroom_id FROM fundraiser_briefs WHERE id = $1`, [id]);
  ...
  if ('edited_output' in body && body.edited_output !== undefined) {
    ...
    if (!body.status) {
      // bump status to 'edited' on first edit
      values.push('edited');
  ...
  const { rows } = await pool.query(
    `UPDATE fundraiser_briefs SET ${updates.join(', ')} WHERE id = $${values.length} RETURNING *`,
    values
  );
  return NextResponse.json({ brief: rows[0] });
}
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await pool.query(`DELETE FROM fundraiser_briefs WHERE id = $1`, [id]);
